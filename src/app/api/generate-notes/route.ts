...
import { getPRTools } from '@/lib/pr-tools';
    ...
    (async () => {
      try {
        for (const pr of relevantPRs) {
          try {
            // Get additional PR information using tools with retry
            const prTools = await retryWithBackoff(() => getPRTools(pr));
            
            // Send tools data first
            await writer.write(
              encoder.encode(
                `data: ${JSON.stringify({ 
                  prId: pr.id, 
            ...
            // Generate developer notes with retry
            const devStream = await retryWithBackoff(() => 
              openai.chat.completions.create({
                ...
                messages: [
                  ...
                  Related Issues:
                  ${prTools.relatedIssues.map(issue => `- ${issue}`).join('\n')}

                  Top Contributors:
                  ${prTools.contributors.map(c => `- ${c.name} (${c.contributions} contributions)`).join('\n')}`
                  }
                ],
                stream: true,
              })
            ...
            try {
              ...
              // Generate marketing notes with retry
              const marketingStream = await retryWithBackoff(() =>
                openai.chat.completions.create({
                  model: "gpt-4.1-mini",
                  messages: [
                    {
              ...
              // Don't throw here, continue with next PR
              continue;
            }
          } catch (prError) {
            console.error("Error processing PR:", prError);
